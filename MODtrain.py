...
def main(args):
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        #val_map_05.append(stats[1]) #MOD dan error al haber eliminado coco log por incompatibilida de la version 
        #val_map.append(stats[0]) # de pycocotools que debería ser inferior a 2.0.9 y no ninguna version de
                                  # Matched con esta antigua versión
        ...
        """
        # SE QUITA HASTA aclarar lo del pycoco porque sin el no funciona el best model
        # y me parece que impide la grabacion de los modelos con cada epoch
        ...
        save_best_model( 
            model, 
            # val_map[-1], # da un error de indice con el -1 se referiría a que es el último elemento
                          # por lo que parece que val_map está vacía
            0.5, # MOD esta linea añadida ya que es un argumento posicional requerido
                                          # y se ha eliminado val_map
            epoch, 
            OUT_DIR,
            data_configs,
            args['model']
