...
def read_boxes_from_xml(xml_file):
...
def main(args):
    ...
    for i in range(len(test_images)):
        ...
        annotations_name=test_images[i][0:len(test_images[i])-3]+"xml"
        #print(image_name)
        ...
        annotations=read_boxes_from_xml(annotations_name)
        print(annotations)
        if len(annotations)==0: print("ERROR NO HAY ANNOTATIONS")
        for j in range(len(annotations)):
            xmin, ymin, xmax, ymax=annotations[j]
            ...
            # Draw the rectangle
            orig_image = cv2.rectangle(orig_image, start_point, end_point, color, thickness)
